...
<form id="frm-mensaje" action="<?= base_url('mensajes/enviar'); ?>" enctype="multipart/form-data" method="post" accept-charset="utf-8" >
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-10">
      <select data-placeholder="Seleccione uno o varios Destinatarios" class="chosen-select ancho" multiple tabindex="4" id="destinatarios" name="destinatarios[]" class="chosen-container" >
        ...
        {
        ?>
            <option value="<?= $fila -> id_rol ?>" <?php if ( $fila -> id_rol == $id_remitente ){ echo "selected"; }?>

          ><?= $fila -> nombre_rol ?></option>
        <?php
        }
        ?>
      </select>
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control" id="asunto" name="asunto" placeholder="" value="<?php echo "Respuesta a: ".$asunto; ?>"required>
    </div>
  </div>
  <div class="form-group row">
    ...
    <select id="id_situacion" name="id_situacion" type="text" class="form-control form-control-sm col-sm-10 "  >
      ...
      {
      ?>
          <option value="<?= $fila -> id_situacion ?>" <?php if ($fila -> id_situacion == $situacion){ echo "selected";} ?> >
            <?= $fila -> nombre ?></option>
      <?php
      }
      ?>
  ...
  <div class="form-group row">
    ...
    <textarea name="mensaje" rows="8" class="col-sm-2 " id="mensaje">"<?= $contenido ?>"</textarea>
  </div>
  <div class="form-group row">
    <div class="col-md-2">
